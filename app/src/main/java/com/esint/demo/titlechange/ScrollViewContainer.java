...
/**
 * Created by Linsa on 2017/3/23:17:49.
 * des:  包含两个ScrollView的容器
 */
...
public class ScrollViewContainer extends RelativeLayout {
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            if (mMoveLength != 0) {
                if (state == AUTO_UP) {
                    mMoveLength -= SPEED;
                    if (mMoveLength <= -mViewHeight) {
                        mMoveLength = -mViewHeight;
                        state = DONE;
                        mCurrentViewIndex = 1;
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (canPullUp && mCurrentViewIndex == 0 && mEvents == 0) {
                    ...
                    if (mMoveLength > 0) {
                        mMoveLength = 0;
                        mCurrentViewIndex = 0;
                    } else if (mMoveLength < -mViewHeight) {
                        mMoveLength = -mViewHeight;
                        mCurrentViewIndex = 1;
                        if(isTuninginterface){
                            isTuninginterface=false;
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mMoveLength == 0 || mMoveLength == -mViewHeight)
                    break;
                if (Math.abs(mYV) < 500) {
                    // 速度小于一定值的时候当作静止释放，这时候两个View往哪移动取决于滑动的距离
                    if (mMoveLength <= -mViewHeight / 2) {
                        state = AUTO_UP;
                    } else if (mMoveLength > -mViewHeight / 2) {
                        state = AUTO_DOWN;
                    }
                } else {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        topView.layout(0, (int) mMoveLength, mViewWidth,
                topView.getMeasuredHeight() + (int) mMoveLength);
        bottomView.layout(0, topView.getMeasuredHeight() + (int) mMoveLength,
                mViewWidth, topView.getMeasuredHeight() + (int) mMoveLength
...
//            Log.i("Tag","------getScrollY------"+ sv.getScrollY()+"");
//            Log.i("Tag","------getMeasuredHeight---getChildAt-"+sv.getChildAt(0).getMeasuredHeight()+"");
//            Log.i("Tag","------getMeasuredHeight---sv---"+sv.getMeasuredHeight()+"");
